...
<body>
    <nav class="navbar navbar-default title1">
        ...
        <a class="navbar-brand" href="#"><b><?php echo $_SESSION['adm_number']?></b></a>
        </div>
            ...
            <div class="col-md-12">
                <?php if(@$_GET['q']==1) 
                {
                    ...
                    while($row = mysqli_fetch_array($result)) {
                        ...
                        $sahi = $row['sahi'];
                        $eid = $row['eid'];
                    $q12=mysqli_query($con,"SELECT score FROM history WHERE eid='$eid' AND Admission='$adm'" )or die('Error98');
                ...
                <?php
                    ...
                    {
                        $eid=@$_GET['eid'];
                        $q=mysqli_query($con,"SELECT * FROM history WHERE eid='$eid' AND Admission='$adm' " )or die('Error157');
                        ...
                        $q=mysqli_query($con,"SELECT * FROM rank WHERE  Admission='$adm' " )or die('Error157');
                        while($row=mysqli_fetch_array($q) )
                        {
                            $s=$row['score'];
                ...
                <?php
                    if(@$_GET['q']== 2) 
                    {
                        $q=mysqli_query($con,"SELECT * FROM history WHERE Admission='$adm' ORDER BY date DESC " )or die('Error197');
                    ...
                    if(@$_GET['q']== 3) 
                    {
                        ...
                        while($row=mysqli_fetch_array($q) )
                        {
                            $e=$row['Admission'];
                            $s=$row['score'];
                            $q12=mysqli_query($con,"SELECT * FROM students WHERE adm_number='$e' " )or die('Error231');
