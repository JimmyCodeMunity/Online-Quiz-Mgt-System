<?php
  ...
  if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) 
  {
    ...
    if($ans == $ansid)
    {
      ...
      if($sn == 1)
      {
        $q=mysqli_query($con,"INSERT INTO history VALUES('$adm','$email','$eid' ,'0','0','0','0',NOW())")or die('Error');
      }
      $q=mysqli_query($con,"SELECT * FROM history WHERE eid='$eid' AND Admission='$adm' ")or die('Error115');
      while($row=mysqli_fetch_array($q) )
      {
        $s=$row['score'];
        $r=$row['sahi'];
      ...
      $r++;
      $s=$s+$sahi;
      $q=mysqli_query($con,"UPDATE `history` SET `score`=$s,`level`=$sn,`sahi`=$r, date= NOW()  WHERE  Admission = '$adm' AND eid = '$eid'")or die('Error124');
    } 
    else
    {
      ...
      if($sn == 1)
      {
        $q=mysqli_query($con,"INSERT INTO history VALUES('$adm','$email','$eid' ,'0','0','0','0',NOW() )")or die('Error137');
      }
      $q=mysqli_query($con,"SELECT * FROM history WHERE eid='$eid' AND Admission='$adm' " )or die('Error139');
      while($row=mysqli_fetch_array($q) )
      {
        $s=$row['score'];
        $w=$row['wrong'];
    ...
    else if( $_SESSION['key']!='suryapinky')
    {
      ...
      $q=mysqli_query($con,"SELECT * FROM rank WHERE Admission='$adm'" )or die('Error161');
      $rowcount=mysqli_num_rows($q);
      if($rowcount == 0)
      {
        $q2=mysqli_query($con,"INSERT INTO rank VALUES('$adm','$email','$s',NOW())")or die('Error165');
      }
      else
      {
        while($row=mysqli_fetch_array($q) )
        ...
        $sun=$s+$sun;
        $q=mysqli_query($con,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE Admission= '$adm'")or die('Error174');
      }
  ...
  if(@$_GET['q']== 'quizre' && @$_GET['step']== 25 ) 
  {
    ...
    $n=@$_GET['n'];
    $t=@$_GET['t'];
    $q=mysqli_query($con,"SELECT score FROM history WHERE eid='$eid' AND Admission='$adm'" )or die('Error156');
    while($row=mysqli_fetch_array($q) )
    {
      $s=$row['score'];
    }
